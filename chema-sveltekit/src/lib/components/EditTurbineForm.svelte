<script lang="ts">
  ...
  import type { WindTurbine, WindTurbineUpdate, Manufacturer } from '$lib/types';
  ...
  // Form data - Initialize with turbine data
  let formData: WindTurbineUpdate = {
    name: turbine.name,
  ...
  // Validation rules (same as AddTurbineForm)
  ...
  function handleFieldChange(field: string, value: any) {
    ...
    if (field.includes('.')) {
      ...
      formData[parent as keyof WindTurbineUpdate][child] = value;
    } else {
      formData[field as keyof WindTurbineUpdate] = value;
    }
  ...
  function validateForm(): boolean {
    ...
    fieldsToValidate.forEach(field => {
      ...
      if (field.includes('.')) {
        const [parent, child] = field.split('.');
        value = formData[parent as keyof WindTurbineUpdate][child];
      } else {
        value = formData[field as keyof WindTurbineUpdate];
      }
...
<form on:submit|preventDefault={handleSubmit} class="turbine-form">
  ...
  <div class="edit-warning">
    ...
    Editing turbine: <strong>{turbine.name}</strong>
  </div>
